...
<body>
	...
	<form action="/delete_my_comment" method="post" id="myForm">
    <table>
        <tr>
		...
		<c:otherwise>
			...
			<tr>
				<td class="checkbox"><input type="checkbox" name="comment_idx" class="post-item" value="${k.comment_idx }"></td>
				<td>${k.comment_idx }</td>
				<td ><a href="/detail?post_idx=${k.post_idx}">${k.comment_content}</a></td>
				<td>${k.comment_updated_at}</td>
			</tr>
			</c:forEach>
		</c:otherwise>
		</c:choose>
...
$(document).ready(function() {
	localStorage.setItem("activeLinkId", "myComments");
    ...
    $("#myComments").closest(".ul2").addClass("active-parent");
});
